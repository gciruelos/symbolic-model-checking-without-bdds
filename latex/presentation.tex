...
{\bf Definition 1.} Una estructura de Kripke es una tupla $M = (S, I, T, \ell)$ con un conjunto finito de
...
\frame{\frametitle{Definiciones...}
{\bf Definition 2.} Semántica. Dada una estructura de Kripke $M$, un camino $\pi$ en $M$, $f$ una fórmula de LTL. Entonces $\pi \models f$ ($f$ es válido a lo largo de $\pi$) se define:
\bigskip
...
{\bf Definition 3 (Validez).} Una fórmula LTL $f$ es universalmente válida en una estructura de Kripke $M$ ($M \models \allp f$) si y solo si $\pi \models f$ para todos los caminos $\pi$ en $M$ con $\pi_0 \in I$.
Una fórmula LTL $f$ es existencialmente válida en una estructura de Kripke $M$ ($M \models \existsp f$) si y solo si existen un camino $\pi$ en $M$ con $\pi_0 \in I$ tal que $\pi \models f$.
...
\frame{\frametitle{Loops}
\includegraphics[width=10cm]{images/fig2.png}
{\bf Definición 4.} Para $l \leqslant k$ llamamos a un camino $\pi$ un $(k,l)$-loop si $\pi_k \to \pi_l$
...
Dada una fórmula LTL $f$ y dados $k, i \in \mathbb{N}$ tales que $i \leqslant k$.
\begin{itemize}
...
{\bf Definición 12. (Sucesor en un loop). } Sea $k, l, i, \in \mathbb{N}$, con $l, i \leqslant k$.
...
Dada una fórmula LTL $f$ y dados $k, l, i \in \mathbb{N}$ tales que $l, i \leqslant k$.
...
\begin{itemize}
  ...
  \item $\ldcorch{\squarep f}^i_k \eqdef \bigwedge\limits_{j=min(i,k)}^k \ldcorch{f}_k^j$
  \item $\ldcorch{\lozp f}^i_k \eqdef \bigvee\limits_{j=min(i,k)}^k \ldcorch{f}_k^j$
...
Dados $k,l \in \mathbb{N}$ tales que $l \leqslant k$ entonces $\prescript{}{l}{L}_k \eqdef T(s_k, s_l)$, $L_k \eqdef \bigvee\limits_{l=0}^k \prescript{}{l}{L}_k$.
}
...
estados de $M$, entonces $M \models \existsp f$ si y solo si existe $k \leqslant M \cdot 2^{|f|}$ tal que $M \models_k \existsp f$.
\bigskip
...
diámetro del bucle de $M$, entonces $M \models \existsp f$ si y solo si existe $k \leqslant u + v$ tal que
